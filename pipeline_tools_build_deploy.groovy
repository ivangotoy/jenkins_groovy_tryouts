	...
	stages {
		...
		stage('PHASE3: BUILD & DEPLOY TOOLS') {
			steps {
				...
				dir('goreleaser') {
					sh 'go mod tidy'
					sh 'go build -ldflags="-s -w" -trimpath -pgo=auto -o goreleaser .'
				...
				dir('go-containerregistry') {
				    ...
				    def Command4 = """sed -i '13s/^/  - GOEXPERIMENT=newinliner\\n/' .goreleaser.yml"""
				    def Command5 = """sed -i '17s/^/  - -pgo=auto\\n/' .goreleaser.yml"""
					sh 'go mod tidy'
					sh Command1
					sh Command2
					sh Command3
				...
				def Command3 = """sed -i '12s/^/      - GOEXPERIMENT=newinliner\\n/' goreleaser.yml"""
				def Command4 = """sed -i '6s/^/    flags:\\n/' goreleaser.yml"""
				def Command5 = """sed -i '7s/^/      - -trimpath\\n/' goreleaser.yml"""
				def Command6 = """sed -i '8s/^/      - -pgo=auto\\n/' goreleaser.yml"""
				sh 'go mod tidy'
				sh Command1
				sh Command2
				sh Command3
